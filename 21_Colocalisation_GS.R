...
tmp1=as.data.frame(fread(paste0("/Cluster_Filespace/Marioni_Group/Rob/EWAS_Disease_GS/mQTLs/Outputs/", as.character(tmp[1,"Probe"]), ".phen.fastGWA.rsid")))
...
# create MAF columns 
tmp2$MAF <- ifelse(tmp2$AF1 > 0.5, 1-tmp2$AF1, tmp2$AF1)
...
# Subset to MAF over 0.01 
tmp2=tmp2[tmp2$MAF>0.01,]
dis2=dis2[dis2$MAF>0.01,]
...
write.csv(output, "/Cluster_Filespace/Marioni_Group/Rob/EWAS_Disease_GS/Cleaned_Results/incident_coloc.csv",row.names=F)
...
output=output[which(!is.na(output$CpG)),]
write.csv(output, "/Cluster_Filespace/Marioni_Group/Rob/EWAS_Disease_GS/Cleaned_Results/prevalent_coloc.csv",row.names=F)



a=read.csv("incident_hyprcoloc.csv")
b=read.csv("incident_hyprcoloc_godmc.csv")
b$Overlap=paste(b$CpG,b$Trait,sep="_")
a$Overlap=paste(a$CpG,a$Trait,sep="_")
a$Type="GS"
b$Type="GoDMC"
t=a[-which(a$Overlap %in% b$Overlap),]
t1=rbind(t,b)
t1=t1[order(t1$Trait),]
nrow(t1)
...
a=read.csv("incident_coloc_full.csv")
b=read.csv("incident_hyprcoloc_full.csv")
cor.test(a$PP.H4, b$regional_probability)

c=read.csv("prevalent_coloc_full.csv")
d=read.csv("prevalent_hyprcoloc_full.csv")
cor.test(c$PP.H4, d$regional_probability)